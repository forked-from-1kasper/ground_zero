...
@[hott] def add_zero : Π (a : ℕ), add nat.zero a ⇝ a
|    0    := <i> nat.zero
| (a + 1) := <i> nat.succ (add_zero a # i)

@[hott] def add_succ (a : ℕ) : Π (b : ℕ), add (nat.succ a) b ⇝ nat.succ (add a b)
|    0    := <i> nat.succ a
| (b + 1) := <i> nat.succ (add_succ b # i)

@[hott] def add_zero_inv : Π (a : ℕ), a ⇝ add a nat.zero :=
Path.refl

@[hott] def add_comm (a : ℕ) : Π (b : ℕ), add a b ⇝ add b a
|    0    := <i> (add_zero a) # −i
| (b + 1) := Path.kan
...
@[hott] def add_assoc (a b : ℕ) : Π (c : ℕ), add a (add b c) ⇝ add (add a b) c
|    0    := <i> add a b
| (c + 1) := <i> nat.succ (add_assoc c # i)

@[hott] def add_comm₃ {a b c : ℕ} : add a (add b c) ⇝ add c (add b a) :=
let r : add a (add b c) ⇝ add a (add c b) :=
<i> add a (add_comm b c # i) in
Path.kan (add_comm a (add c b))
  (<j> r # −j) (<j> add_assoc c b a # −j)

@[hott] example (n m : ℕ) (h : n ⇝ m) : nat.succ n ⇝ nat.succ m :=
<i> nat.succ (h # i)
