...
@[hott] def network.decode {α : Type u} (H : prop α) : network α → α :=
begin
  fapply graph.ind,
  { exact id },
...
@[hott] def network.prop {α : Type u} (H : prop α) : prop (network α) :=
begin
...
@[hott] def network.proplem {α : Type u} (H : prop α) : α ≃ network α :=
begin
